"""
This conftest.py contains useful components for preparing SparkSession and other Databricks objects.
"""
...
class FileInfoFixture:
    """
    Class used to mock the DBUtils FileInfo object
    """
    path: str
    name: str
    size: int
...
class DBUtilsFixture:
    """
    Class used for mocking the behaviour of DBUtils inside tests.
    """
...
def spark() -> SparkSession:
    """
    Fixture providing preconfigured SparkSession with Hive and Delta support.
...
def mlflow_local():
    """
    Fixture providing local instance of MLflow with support for tracking and registry functions.
...
def dbutils_fixture() -> Iterator[None]:
    ...
    with patch('churn.common.get_dbutils', lambda _: DBUtilsFixture()):
        yield
